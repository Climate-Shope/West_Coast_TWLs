%Script to intepolate 10-m spaced minor profile elevations

%Part 1 casts transects
%Part 2 (This script) interpolates the profile
...
%% 
dirout      = ['E:\West_Coast_TWL_Hazards\03_Results\' NAME_GRD_transects '\interp_minor_profiles_wave_model']; if ~exist([dirout],'dir'), mkdir(dirout); end
...
transects_mod = Transects4; 

INCORRECT_RUNS = [];
Ntr=length(transects_mod.OBJECTID);
for ii = 1:Ntr;
    ...
    mmhhww=linspace(MHW(Transects4.OBJECTID(ii),3),MHW(Transects4.OBJECTID(ii)+1,3),11);
    mmhhww([1;11])=[];
    mmssll=linspace(MSL(Transects4.OBJECTID(ii),3),MSL(Transects4.OBJECTID(ii)+1,3),11);
    mmssll([1;11])=[];
    ...
    uu=GRIDid(Transects4.OBJECTID(ii));
    ...
    depth=depthi;
    %First create a cut of elevation 
   
    %Remove any potential islands in front of the profile by finding any
    %reigons that are above MSL but smaller than 50m 
   
    temp_0=find(depth<msl);
    if ~isempty(temp_0);
    temp=diff(temp_0);
    test=find(temp>50)-1;
    %test2=test+temp(temp>50);
    if~isempty(test)
        if test(1) ==0;
            test(1)=1;
        end
   ...
   cutoff =mhw; %Anything above this mark will not be considered 
...
wave_profile.ycoast = wave_profile.ypf(1); 
    ...
    %% save 
    fout = [dirout, filesep, 'profile_', dec2base(Transects4.OBJECTID(ii),10,4) ,'_', num2str(Transects4.MINORID(ii)), '.mat']; 
