%Troubleshoot the remaining profiles that did not run because Beach slopes
%were listed as NaN 
...
tgg=find(strcmp(Run_Success.Complete,'Fail'));

for ii=ii_grid_array;
    OBJECTID=dec2base(ii,10,4);
    try
        load([DirOut filesep 'TWL_Data_Transect_' num2str(ii)]);
        if ~isnan(TWL_Data.RunupMethod.StockSlope)
            continue
        end
        clear TWL_Data
    ...
    if isnan(SlopeInfo.Runup_Method.StockSlope); %if NaN, select the closest profile with a non nan value
   ...
   if SlopeInfo.toe_loc==1
    ...
    name=[DirOut filesep 'Fix_Success']; 
