...
DirOut = ['F:\West_Coast_TWL_Hazards\03_Results\' NAME_GRD_transects '\extreme_value_analysis_update_10_12_2019'];
if ~exist(DirOut,'dir'), mkdir(DirOut); end
DirOut_TWL = ['F:\West_Coast_TWL_Hazards\03_Results\' NAME_GRD_transects '\TWL_Output_update_10_12_2019'];
...
Val=[];
for kk=1:length(Socal);

load(['F:\West_Coast_TWL_Hazards\03_Results\Return_Levels_10_2_2019\' Socal '_RP10.mat']);
...
clear RP10
load(['F:\West_Coast_TWL_Hazards\03_Results\Return_Levels_10_2_2019\' NAME_GRD_transects '_RP10.mat']);
...
for ii = ii_grid_array
    ...
    %Load Extreme values 
    try
        AA=load(['F:\West_Coast_TWL_Hazards\03_Results\' NAME_GRD_transects '\extreme_value_analysis_10_2_2019\Extrema_GPD_',OBJECTID,'.mat']);
    catch
        AA=load(['F:\West_Coast_TWL_Hazards\03_Results\' NAME_GRD_transects '\extreme_value_analysis_10_2_2019\Extrema_prf',OBJECTID,'.mat']);
    end
    
    load([TWL_Dir filesep 'TWL_Data_Transect_' num2str(ii) '.mat']);
...
%first change, if exceed slope is <36, use stockdon as determined through
%testing
    if Composite_Slope.wall_slope<36 && ~strcmp(RunupMethod.ESI,'6B') && strcmp(RunupMethod.Method,'Stockdon') 
...
%% reassess morphology for the composite slope used for TAW for a few specific problem scenarios
    if Composite_Slope.wall_slope<45 && strcmp(RunupMethod.ESI,'6B') && SlopeInfo.depth(SlopeInfo.overtop_point)<13 
    ...
    if Composite_Slope.wall_slope<45 && SlopeInfo.depth(SlopeInfo.overtop_point)<13  && strcmp(RunupMethod.ExceedMethod,'TAW'); 
       zz=diff(diff(SlopeInfo.depth));
  ...
  %% New TWL run  
    ...
    [Runup,Runup_Mag,type,DWL] = CalcRunup_Fix6B(Wavecon, SlopeInfo, RunupMethod, Composite_Slope,tide); 
    ...
    TWL_Data.Runup=Runup; %<--- Again this is the TWL
...
%% Now redo the extreme value analysis using new TWLs
ii_ind=Flag;
...
for ii = ii_ind
    TRANSID = ii; 
    OBJECTID=dec2base(ii,10,4);
    fts=['F:\West_Coast_TWL_Hazards\03_Results\' NAME_GRD_transects '\TWL_Output_update_10_12_2019\TWL_Data_Transect_',num2str(ii),'.mat'];
