function [Out]=Runup_Method(SlopeInfo,wave_profile,Rey,fslope);
...
%toe_loc = location of toe of the structure, cliff, dune based on what the
%water interacts with from stockdon runp methodology 
...
%slope_break_locs = CliffJun_Loc = locations of the slope breaks in the profile, need to
...
elseif Stocktoe < 1; %Here the slope is defined as MHW to the maxi or cliffjun
...
%% Fill parameter if doesn't exit
...
%analysis (NHC 2005).
if strcmp(ESI,'1A') || strcmp(ESI,'1B')|| strcmp(ESI,'2A')
    gamma=1.0;
    Method='TAW';
...
if SlopeInfo.depth(SlopeInfo.CliffJun_Loc)-SlopeInfo.depth(toe_loc) > 10
    slope=(SlopeInfo.depth(SlopeInfo.CliffJun_Loc)-SlopeInfo.depth(toe_loc))/(SlopeInfo.CliffJun_Loc-toe_loc);
    if slope>0.67
       two='TAW';
    else
        two='Stockdon';
...
%Need to modify this a bit for specific ESIs like 6B and 5
...
%% Now check for instaces where there is a stockdon slope defined, but poorly defined because of the short length
%  and redefine the stockdon slope based on initial input
if strcmp(Method,'Stockdon') && strcmp(ESI,'6B') %riprap
