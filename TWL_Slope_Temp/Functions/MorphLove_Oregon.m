function [Out]=MorphLove_Oregon(wave_profile,ESI,ESI_Array,Rey,OBJECTID,UTM);
...
if isempty(Rey) %i.e. not a seawall focused profile
    ...
    %Create a flag if the beach is very wide
    longbeach=find(wave_profile.depth>6);
    if ~isempty(longbeach)
        longbeach=longbeach(1);
    else
        longbeach=1;
    end
    ...
    elseif ~strcmp(ESI,'3A') && ~strcmp(ESI,'4')
        if longbeach<90
            ...
            [pks,locs] = findpeaks(result(2,:),0.5,1,4); %Must have a 1 m prominence
        end
    else
    ...
    %elevation, redoc the profile simplification and search for new loacations
    %
...
%%
%Next, limit the extent of the profile such that it is always in a
...
%% If there is a seawall 
   ...
   %Approximate the locaiton along the profile of the seawall location 
   distance=sqrt(xcomp+ycomp);
   %define the maximum as the location of the seawall    
   [~,maxi]=min(abs(distance));
    maxi_org=maxi;
    
    %redo profile simplificationand finding the maximum
...
%% Now find maximum if maxivalue is empty
...
%Next, limit the extent of the profile such that it is always
...
if~isempty(toe); 
  ggg=movingslope(depth2,3); hhh=movingslope(ggg,3); %Basically computes second detivative)
  if depth2(maxi)<8 && strcmp(ESI,'3A') || depth2(maxi)<8 && strcmp(ESI,'4')||depth2(maxi)<13 && maxi>85 && strcmp(ESI,'3A') || depth2(maxi)<13 && maxi>85 && strcmp(ESI,'4')
...
if toe<MHW_loc && MHW < 10;
 ...
 if ~isempty(Rey);
     if change_riprap==0; %Should not need to modify riprap in fron, code should pick it up
         depth3=depth2;
         depth3(maxi+1:end)=[];
...
elseif strcmp(ESI,'3A') && maxi>78 && wave_profile.depth(maxi)>15|| ~strcmp(class(ESI),'char') && maxi>78 && wave_profile.depth(maxi)>15
    [pks,locs] = findpeaks(b,[],1,0.1);
elseif strcmp(ESI,'3A') && maxi>78 && wave_profile.depth(maxi)<15|| ~strcmp(class(ESI),'char') && maxi>78 && wave_profile.depth(maxi)<15
    [pks,locs] = findpeaks(b,[],1,0.05);
else
...
for mm=1:length(locs);
    ...
    nn=wave_profile.depth(toe:locs(mm));
...
%now with the new toe, find a new maximum 
result = DouglasPeucker([wave_profile.L;wave_profile.depth],0.1);
fepth=interp1(result(1,:),result(2,:),wave_profile.L); 
[pks,locs] = findpeaks(fepth,[],1,3.5);
...
elseif strcmp(ESI,'3A') && strcmp(ESI_Array(ESI_Better,5),'3A: Fine to Medium Grained Sand Beaches') && toe>55;
    [~,i] = findpeaks(Values,[],5,-20);
    if ~isempty(i)
    i=i(1);
    else
...
elseif isempty(bbb) && strcmp(ESI,'5')
    [pks,locs] = findpeaks(Values,[],20,-8);
elseif isempty(bbb);
    if toe>90 && strcmp(ESI,'3A') && wave_profile.depth(maxi)>20
        X=[toe length(depth4)];
        V=[depth4(toe) depth4(end)];
        Xq=[toe:length(depth4)];
        Vq = interp1(X,V,Xq);
        Values2=depth4(toe:end)-Vq;
        
        
    [pks,locs] = findpeaks(Values2,[],20,-4);
...
if ~isempty(locs)
    if length(locs)>1 
        i=locs(end-1);
    elseif length(locs)==1 && locs~=maxi
        i=locs;
    elseif exist('zipzip','var')
        i=locs(1);
    end
end
if exist('Values2','var')
    val=Values2(i-toe);
else
val=Values(i);
end
...
if val<0
    if exist('Values2','var')
        [pks,locs] = findpeaks(Values2,[],10,cut);
        locs=locs+toe;
    else
...
elseif strcmp(ESI,'3A') && exist('Values2','var')
    [red,green] = findpeaks(Values2,0.1,10,-6);
    green=green+toe;
...
elseif strcmp(ESI,'6B') && toe<10
    green(wave_profile.depth(green)<4)=[];
end
if ~isempty(green)
        ...
        if toe>5 && toe>MHW_loc
            ...
            if~isempty(c)
                ...
                elseif c(1)< toe-3;
                   ...
                   %If the toe changes, need to look at the cliff junction
...
%% 
if ~strcmp(ESI,'3A') && ~strcmp(ESI,'4') && ~strcmp(ESI,'6B') && isempty(Rey)%no big rocks in the way in riprap the rocks are pre-installed
...
if wave_profile.depth(cliffjun_loc)<9 && ~strcmp(ESI,'3A') && ~strcmp(ESI,'4') && length(wave_profile.depth)<100
...
%Account for where the cliffjun really isnt a slope break 
...
%% add piece to account for a beach if it is not included in the ESI
if point1>30 && ~strcmp(class(ESI),'char')
    ESI='3A';
end
...
if slopemax>slopejun && maxi-cliffjun_loc<80 && toe<100 && ~strcmp(ESI,'6B') || slopemax>slopejun+10 && slopemax>20 && maxi-cliffjun_loc<50 && toe<120 && strcmp(ESI,'3A') || maxi-cliffjun_loc<10 && slopejun>63 && slopemax>63 && slopeint>63 && ~strcmp(ESI,'6B');
    overtop_point=maxi;
else
    %here compare the slopes
    slopediff=slopejun-slopemax;
    
    if slopediff < 10 && maxi-cliffjun_loc<10 && wave_profile.depth(maxi)-wave_profile.depth(cliffjun_loc)<4.5 || slopediff < 15 && maxi-cliffjun_loc<10 && wave_profile.depth(maxi)-wave_profile.depth(cliffjun_loc)<4 || slopediff < 10 && wave_profile.depth(maxi)<7 && maxi-cliffjun_loc<90; %added second half 4/16/2019
        overtop_point=maxi;
    elseif strcmp(ESI,'3A') && slopemax>50 && slopeint>45;
        overtop_point=maxi;
    elseif strcmp(ESI,'3A') && slopemax<10 && slopejun<15 && slopeint<5 && maxi-cliffjun_loc>45;
        overtop_point=maxi;
    elseif strcmp(ESI,'3A') || ~strcmp(class(ESI),'char') && slopejun<17 && slopemax<15;
       if slopediff < 10 && maxi-cliffjun_loc<10 || slopediff < 15 && maxi-cliffjun_loc<20 && wave_profile.depth(maxi)-wave_profile.depth(cliffjun_loc)<4 || slopediff < 10 && wave_profile.depth(maxi)<7 && maxi-cliffjun_loc<90; %added second half 4/16/2019
          overtop_point=maxi;
       elseif slopeint<15 && maxi-cliffjun_loc<30 && wave_profile.depth(maxi)>9 && wave_profile.depth(cliffjun_loc)>6.75 && wave_profile.depth(maxi)-wave_profile.depth(cliffjun_loc)<6;
          overtop_point=maxi;  
       elseif slopeint<10 && maxi-cliffjun_loc<45 && wave_profile.depth(maxi)>10 && wave_profile.depth(cliffjun_loc)>6 && wave_profile.depth(maxi)-wave_profile.depth(cliffjun_loc)<6;
          overtop_point=maxi; 
       elseif slopediff<5 && abs(slopemax-slopeint)<3
           overtop_point=maxi; 
       else
          overtop_point=cliffjun_loc;
       end
    elseif strcmp(ESI,'2A')    %The idea here is if the cliffjun is a maximum, its likely just a rock in the way
    ...
    elseif strcmp(ESI,'1A') && slopemax>70;
        overtop_point=maxi;     
    elseif strcmp(ESI,'1A') && slopeint<20 && wave_profile.depth(maxi)<20;
        if length(cliffjun_loc:maxi)>5
            if min(detrend(wave_profile.depth(cliffjun_loc:maxi)))>-2 && max(detrend(wave_profile.depth(cliffjun_loc:maxi)))<2.2
            overtop_point=maxi;
            else
            overtop_point=cliffjun_loc; 
            end
    ...
    elseif strcmp(ESI,'5') && slopemax>50 && slopeint>50;
        overtop_point=maxi;   
     elseif strcmp(ESI,'5') && slopemax>30 && wave_profile.depth(maxi)>18
         if length(toe:maxi)>10
            if min(detrend(wave_profile.depth(toe:maxi)))>-2.5 && max(detrend(wave_profile.depth(toe:maxi)))<2.5 && slopejun-slopeint <11
                overtop_point=maxi;
            else
                overtop_point=cliffjun_loc; 
            end
    ...
    elseif strcmp(ESI,'6B') && abs(slopediff)<7 && slopemax>20 && maxi-cliffjun_loc<20 && slopeint>slopejun;
        ...
        if slopediff<3 && abs(slopejun-slopeint)<3 && abs(slopemax-slopeint)<3 && slopejun>40;
          overtop_point=maxi;   
        else
        overtop_point=cliffjun_loc;
        end
...
if wave_profile.depth(maxi) < 15 && maxi-cliffjun_loc < 15 && wave_profile.depth(maxi)>wave_profile.depth(cliffjun_loc); 
    overtop_point=maxi;
else
...
if strcmp(ESI,'3A') && overtop_point==maxi && wave_profile.depth(cliffjun_loc)>6 && slopeint<15 && cliffjun_loc>toe; 
...
if strcmp(ESI,'4') && overtop_point-toe>100 && maxi-cliffjun_loc<10 && toe<25
    X=[1 overtop_point];
...
if strcmp(ESI,'3A') && overtop_point-toe>45 && maxi-cliffjun_loc<50 && toe>70 && overtop_point==maxi && wave_profile.depth(maxi)-wave_profile.depth(cliffjun_loc)>20
    X=[1 overtop_point];
    V=[wave_profile.depth(1) wave_profile.depth(overtop_point)];
    Xq=[1:overtop_point];
    Vq = interp1(X,V,Xq);
...
%oriignal DEM extent, so convert to MHW for 3A and 5 beach profiles and -1
...
if exist('toe_onshore','var');
    ...
    if toe_onshore>cliffjun_loc && wave_profile.depth(cliffjun_loc)>cal;
        Out.toe_onshore=Out.toe_loc; 
    else
