function [R,R_mag,type,DWL] = CalcRunup_v7(Wavecon, SlopeInfo, RunupMethod, Composite_Slope,tide);

%Calculate Runup using slope, water level,and runup method inputs
...
%If empty, means no TAW computatino, but filling in the variables to ease
%computation below
...
%% Get deepwater wave conditions to calculate the Stockdon setup and runup conditions
...
%% determine inital runup conditions
    %Use Stockdon Runup formulation
...
if strcmp(RunupMethod.ExceedMethod,'TAW');
    ...
    temp=find(~isnan(ybIb_local));
     ...
     % if low in elevation, need special considerations 
     if toe_d<3
     
     %parameters of these conditions was determined through testing for the
     %region
     if toe>=2 && ~strcmp(ESI,'6B') && Composite_Slope.wall_slope<45 && toe_d>SlopeInfo.MHW;
     ...
     elseif Composite_Slope.wall_slope>45 && toe<2
          ...
          temp6=temp(Composite_Slope.Slope(temp)>1 & Ho(temp)>=Hmo(temp));
          temp7=temp(Composite_Slope.Slope(temp)>1 & Ho(temp)<Hmo(temp));
