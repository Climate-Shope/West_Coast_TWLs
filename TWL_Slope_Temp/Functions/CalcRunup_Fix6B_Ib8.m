function [R,R_mag,type,DWL] = CalcRunup_Fix6B_Ib8(Wavecon, SlopeInfo, RunupMethod, Composite_Slope,tide);

%Calculate Runup using slope, water level,and runup method inputs with
%stirngint modifications for TAW profiles where IB>8
...
%% determine inital runup conditions
    ...
    [R,out1,out2] = Runup(Ho,Tp,S,13); R_mag=R;R_stk=R; R=R+SWL; %Reference it to NAVD88 by adding the Still Water level
    R_guess=R;
...
if strcmp(RunupMethod.ExceedMethod,'TAW');
    ...
    temp=find(~isnan(ybIb_local));
     ...
     if Composite_Slope.wall_slope>45
     ...
     tt=find(Composite_Slope.Slope>1);
     tt=find(ismember(temp5,tt));
     
     temp6a=temp5(tt);
     
     temp6=sort(unique([temp6;temp6a]));
     ...
     %Catch to find where the runup generated it too extreme
     AA=find((hiIb_Run./Hmo(temp5))>3);
